...
/**************************** ixo-client-sdk ****************************/
    const wallet = await ixoclient.makeWallet(ixohelper.mnemonic_ECO)
    ...
    try {
        ...
        const bondDid = "someBondDID"
        const bondToken = "CHIM"
        const reserveToken = "UIXO"
        const amount = "10"
...
/**************************** libraries (@cosmjs/amino + sovrin) ****************************/
    ...
    try {
        ...
        await sovrinhelper.init(ixohelper.mnemonic_user4_investor, ixohelper.prefix)
        const { signed, signature } = await sovrinhelper.sign(
            await messagehelper.getMsgBuyStdSignDoc(sovrinhelper.address, ixohelper.bondDID, ixohelper.buyerDID_user4_investor))
...
/**************************** OPERA ****************************/
    ...
    try {
        ...
        //     await messagehelper.getMsgBuyStdSignDoc(operaed25519helper.address, ixohelper.bondDID, ixohelper.buyerDID_user4_investor))
